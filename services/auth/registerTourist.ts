...
export const registerTourist = async (formData: FormData): Promise<ActionResponse> => {
    try {
        const payload = {
        ...
        const validatedPayload = zodValidatorRequest(payload, registerBaseUserSchema);
        if (!validatedPayload.success) return validatedPayload;

        const res = await serverFetch.post("/user/register/tourist", {
